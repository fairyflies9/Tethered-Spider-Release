			      ...
			      const double dt)
{
  ...
  static const int npts = ceil(2*(line_length/L1)*N1); // number of points along the line
  static const double ds = line_length/(npts-1); // line mesh spacing (meters)
    

  // Find out the Lagrangian index ranges.
  const std::pair<int,int>& line_lag_idxs = l_data_manager->getLagrangianStructureIndexRange(0, finest_ln);
  ...
  for (vector<LNode*>::iterator it = nodes.begin(); it != nodes.end(); ++it)
    {
      ...
      //Point& X_target = force_spec->getTargetPointPosition();
		double& kstiff = force_spec->getStiffness();
